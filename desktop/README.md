...
- 两个凭据由后端捕获后立即从子进程环境删除，不会被 Worker 继承；桌面 Uvicorn 关闭 access log，避免一次性 URL 进入日志。
...
- 用户正常关闭客户端时，监督器先调用一次性凭据保护的本地优雅停止端点，让 FastAPI lifespan、SQLite 和 Worker 完成收尾；超时后才由 Job Object 强制回收。
