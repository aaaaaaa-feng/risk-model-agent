...
15. 多数据版本的页面操作原先默认取列表第一项，无法清楚表达本次探索/建模对象；现在页面显示并选择当前数据版本，活动 Run 期间锁定版本，未启动 Run 时画像分析使用当前版本的本地 Profile 和合规 Y 候选。
...
- `34 passed`（当前本地环境）：单元、Worker、API、半信任恢复、XLSX Sheet、Provider DLP/预算/出站摘要、报告导出、Trace 脱敏与事件链、资源边界、数据字典、校准/稳定性/评分卡映射、训练集筛选、高维分析守卫、类别不平衡训练策略、多数据版本与本地画像、Tool Registry、清洗版本、清洗确认门禁、Baseline/新 OOT 复评和 what-if 隔离、项目多轮对话、报告叙事锁定、聊天文本边界、跨平台打包契约和黄金回归。
