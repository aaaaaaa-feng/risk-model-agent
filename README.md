...
- Tauri 2 Windows 客户端：展示本地启动页、管理冻结后端进程，并在应用窗口内加载工作台；正式安装版不再弹出终端或外部浏览器。
...
Windows 使用 `scripts/build_windows.ps1`，它会依次构建 React 前端、PyInstaller 本地服务、后端完整性清单和 Tauri 2 NSIS 客户端：
