...
- 已实现：Provider Gateway 的 OpenAI-compatible 接口和显式 `llm_enabled` 开关。外部请求只允许别名化 SafeEvidence，并在本地记录可查看的脱敏出站请求摘要/哈希；计划、代码审核失败会保留结构化原因并阻断；代码 Reviewer 使用 AST/依赖/危险调用静态门禁；单 Run/单月 token 预算在本地调用前熔断并记录 usage。没有 Provider 时仍走确定性本地流程。
