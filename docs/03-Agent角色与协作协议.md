...
项目对话由主 Agent 处理，使用当前 Run 的匿名 SafeEvidence 作为上下文；对话不能直接推进状态机。自由文本在可选外部调用前还要经过本地敏感值阻断，命中时只返回确定性回复。报告叙事 Agent 只能基于已验证的结构化产物起草文字，不能计算或改写指标；专家编辑和锁定后，文本版本与证据引用进入报告产物。
