...
Worker 能力通过本地 Tool Registry 暴露给 Orchestrator。V1 优先使用进程内强类型调用或本地 JSON-RPC，不要求 MCP。

- Tool Schema 负责参数校验和权限声明；
- Worker 只接收本地 `dataset_ref`，不接收前端上传的任意路径；
- 结果保存为不可变 Artifact，工具返回摘要与引用；
- Agent 只能看到符合安全策略的结果摘要；
- 未来如需跨产品复用，可在外层增加本地 MCP Adapter，但不能扩大底层权限。
...
AI 生成代码用于可读、可导出和复现。默认训练路径仍由受控 Worker 根据 `ModelPlan` 执行，以降低任意代码风险。
