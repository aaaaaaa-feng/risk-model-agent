...
每个 LangGraph 工具在独立可终止子进程执行。父进程监控真实 RSS 和 wall-clock timeout；超限时结束该子进程并返回明确错误，不用无法强制停止的线程继续占用唯一队列。
...
根据估算内存和可用内存决定列批次、CSV 解析行批次和模型并发。缺失率、画像和 IV 可按列分批后汇总；超过当前完整 DataFrame 安全边界时必须要求列投影、提高预算或明确延后，不能把尚未实现的全量 out-of-core 计算冒充为已支持。资源不足时依次：减小批次、单模型顺序、延后高资源模型、明确失败。禁止静默抽样或删样本。
