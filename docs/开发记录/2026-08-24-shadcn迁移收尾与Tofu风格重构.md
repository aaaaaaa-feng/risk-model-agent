...
- **令牌覆写**:`tokens.css` 追加 `[data-theme="dark"]` 整块同结构覆写——画布 `#17171a`、面板 `#232327`、墨色反为 `#f2f2f0`;accent 紫提亮到 `#8385f7`;状态色改为深底亮字;投影/遮罩加深。
- **行动层分离**:新增 `--action-bg/-text/-hover`。小型 CTA(主按钮、选中页签、当前阶段胶囊、侧栏「新建项目/设置中心」)深色下反转为浅色;`--black` 装饰面板(舞台 hero、弹窗/抽屉头、记分卡、代码块、品牌块、头像)保持纯黑,深色下补 `inset` 细边保持轮廓。
- **黑底文字**:黑底面板上的 `color: var(--paper)` 全部改常量浅色 `--output-text`(paper 在深色下变暗,会不可读)。
- **切换**:新增 `useTheme` hook(localStorage `risk-agent-theme` 持久化,未选择时跟随 `prefers-color-scheme`);`index.html` 内联脚本首帧前写 `data-theme` 防闪屏;头部右侧太阳/月亮 ghost 钮切换。
- 滚动条颜色由硬编码改走 `--scroll-thumb` 令牌。
- 已用 headless Chrome 分别截图验证深色/浅色首屏(深色经系统偏好触发,浅色经临时强制后截图,验证完即还原)。
