...
@router.get("/projects")
def list_projects(include_archived: bool = True, ctx: AppContext = Depends(context)) -> dict[str, Any]:
...
def get_project(project_id: str, ctx: AppContext = Depends(context)) -> dict[str, Any]:
    return {
        ...
        "dataset_versions": ctx.database.list("dataset_versions", {"project_id": project_id}, limit=2000),
        "target_tasks": ctx.database.list("target_tasks", {"project_id": project_id}, order_by="queue_position ASC", limit=2000),
        "runs": [_run_summary(item) for item in ctx.database.list("runs", {"project_id": project_id}, limit=2000)],
    }


@router.patch("/projects/{project_id}")
def update_project(project_id: str, payload: ProjectUpdate, ctx: AppContext = Depends(context)) -> dict[str, Any]:
...
async def upload_asset(
    ...
    if Path(name).suffix.lower() not in SUPPORTED_SUFFIXES:
        raise HTTPException(400, "仅支持 CSV、XLSX、XLSM 和 XLS。")
        ...
        with staged.open("wb") as target:
            while chunk := await file.read(4 * 1024 * 1024):
                ...
                if size > MAX_UPLOAD_BYTES:
                    raise HTTPException(413, "文件超过当前可配置上传上限。")
                target.write(chunk)
...
def get_asset_schema(asset_id: str, ctx: AppContext = Depends(context)) -> dict[str, Any]:
    ...
    return {
        "asset_id": asset_id,
        "columns": [
            {"name": str(column), "type": infer_type(frame[column])}
...
def choose_sheet(asset_id: str, payload: SheetChoice, ctx: AppContext = Depends(context)) -> dict[str, Any]:
    return {"asset": ctx.catalog.choose_asset_sheet(asset_id, payload.sheet)}


@router.post("/data-assets/{asset_id}/dictionary")
def attach_dictionary(asset_id: str, payload: DictionaryAttach, ctx: AppContext = Depends(context)) -> dict[str, Any]:
...
def list_dataset_versions(project_id: str, ctx: AppContext = Depends(context)) -> dict[str, Any]:
    ctx.catalog.get_project(project_id)
    return {"dataset_versions": ctx.database.list("dataset_versions", {"project_id": project_id}, limit=5000)}


@router.get("/dataset-versions/{dataset_version_id}")
def get_dataset_version(dataset_version_id: str, ctx: AppContext = Depends(context)) -> dict[str, Any]:
    return {"dataset_version": ctx.catalog.require("dataset_versions", dataset_version_id)}


@router.get("/join-plans/recommend")
def recommend_join(left_asset_id: str, right_asset_id: str, ctx: AppContext = Depends(context)) -> dict[str, Any]:
...
def execute_join_plan(plan_id: str, payload: JoinExecute, ctx: AppContext = Depends(context)) -> dict[str, Any]:
...
def create_target_tasks(payload: TargetTasksCreate, ctx: AppContext = Depends(context)) -> dict[str, Any]:
    tasks = [
...
def list_target_tasks(project_id: str, ctx: AppContext = Depends(context)) -> dict[str, Any]:
    return {"target_tasks": ctx.database.list("target_tasks", {"project_id": project_id}, order_by="queue_position ASC", limit=5000)}


def _run_summary(run: dict[str, Any]) -> dict[str, Any]:
    return {key: value for key, value in run.items() if key != "state"}
