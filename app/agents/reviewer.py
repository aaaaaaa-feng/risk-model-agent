...
class IndependentReviewer:
    ...
    def review_plan(
        ...
        for item in diagnostics.get("issues", []):
            if item.get("severity") == "blocking":
                issues.append(item)
        if plan.get("split", {}).get("method") == "time_holdout" and not plan.get("split", {}).get("time_column"):
            issues.append({"code": "TIME_COLUMN_REQUIRED", "severity": "blocking", "message": "时间外推必须指定时间字段。"})
        if screening is not None and not screening.get("included"):
            issues.append({"code": "NO_FEATURES_AFTER_SCREENING", "severity": "blocking", "message": "筛选后没有可入模变量。"})
        return self._record("plan", issues, {"train_only": True, "oot_locked": True})
    ...
    def review_execution(self, model_result: dict[str, Any]) -> dict[str, Any]:
        ...
        if not model_result.get("champion"):
            issues.append({"code": "CHAMPION_MISSING", "severity": "blocking", "message": "没有冠军模型。"})
        if model_result.get("oot_used_for_selection") is not False:
            issues.append({"code": "OOT_SELECTION_LEAK", "severity": "blocking", "message": "OOT 被用于模型选择。"})
        for item in model_result.get("candidates", []):
            ...
            if item.get("fit_scope") != "train_cv_only" or item.get("selection_scope") != "test":
                issues.append({"code": "FIT_SCOPE_INVALID", "severity": "blocking", "message": f"{item.get('candidate')} 的拟合/选择范围不合规。"})
            metrics = item.get("test_metrics") or {}
            if metrics.get("roc_auc") is None or metrics.get("ks") is None:
                issues.append({"code": "MODEL_METRICS_MISSING", "severity": "blocking", "message": f"{item.get('candidate')} 缺少 AUC/KS。"})
        return self._record(
            "execution",
            issues,
            {
    ...
    def review_report(self, report: dict[str, Any]) -> dict[str, Any]:
        ...
        if champion.get("candidate") != summary.get("champion"):
            issues.append({"code": "REPORT_CHAMPION_MISMATCH", "severity": "blocking", "message": "摘要与专业详情的 Champion 不一致。"})
        if (champion.get("test_metrics") or {}).get("roc_auc") != summary.get("test_auc"):
            issues.append({"code": "REPORT_METRIC_MISMATCH", "severity": "blocking", "message": "报告中的 Test AUC 不一致。"})
        if report.get("governance", {}).get("oot_used_for_selection") is not False:
            issues.append({"code": "REPORT_OOT_SCOPE_INVALID", "severity": "blocking", "message": "报告未声明 OOT 仅用于最终评估。"})
        ...
        if summary.get("quality_verdict") != expected_verdict:
            issues.append({"code": "REPORT_QUALITY_VERDICT_MISMATCH", "severity": "blocking", "message": "报告质检结论与 Test 排序性不一致。"})
        return self._record(
            "report",
            issues,
            {
    ...
    def llm_review(self, scope: str, safe_evidence: dict[str, Any]) -> dict[str, Any]:
        ...
        if not payload:
            return {
                ...
                "status": "fallback_pass",
                "issues": [],
                "evidence": {"provider_error": result.error_code, "deterministic_review_retained": True},
            }
        issues = payload.get("issues") if isinstance(payload.get("issues"), list) else []
        status = payload.get("status") if payload.get("status") in {"pass", "revise", "block"} else "revise"
        return {
            "scope": scope,
            "status": status,
            "issues": issues,
            "evidence": {"provider": self.gateway.settings.provider, "model": result.model, "payload_hash": result.payload_hash},
        }
    ...
    def combine(scope: str, deterministic: dict[str, Any], llm: dict[str, Any]) -> dict[str, Any]:
        ...
        if any(item.get("severity") == "blocking" for item in issues) or llm.get("status") == "block":
            status = "block"
    ...
    @staticmethod
    def _record(scope: str, issues: list[dict[str, Any]], evidence: dict[str, Any]) -> dict[str, Any]:
        blocking = any(item.get("severity") == "blocking" for item in issues)
        return {"scope": scope, "status": "block" if blocking else ("revise" if issues else "pass"), "issues": issues, "evidence": evidence}
