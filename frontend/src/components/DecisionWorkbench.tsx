...
export function DecisionWorkbench({ run, decision, onResolved, notify }: Props) {
  ...
  useEffect(() => {
    ...
    else if (decision.kind === "confirm_models") setEdits({ models: summary.plan?.models || [], score: summary.plan?.score || {} });
    else setEdits({});
