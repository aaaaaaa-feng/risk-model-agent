...
const stages = ["target_confirmation","data_diagnosis","cleaning","split","screening","binning","model_plan","code_review","training","reporting","completed"];

export function StageRail({ run, decision, events }: { run: Run | null; decision: Decision | null; events: RunEvent[] }) {
  const [history, setHistory] = useState(false);
  if (!run) return <aside className="stage-rail" aria-label="当前运行阶段"><div className="rail-head"><span>RUN STATUS</span><b className="rail-status idle">未启动</b></div><div className="stage-box"><span className="eyebrow">NEXT</span><h2>准备项目数据</h2><p>导入本地表、完成关联并创建 Y 任务后，这里会持续显示 Agent、工具与 Reviewer 状态。</p></div></aside>;
  ...
  return <aside className="stage-rail" aria-label="当前运行阶段">
    ...
    {history&&<div className="rail-history">{events.slice().reverse().map(event=><div key={event.id}><time>{new Date(event.time).toLocaleTimeString()}</time><b>{event.agent} · {event.status}</b><p>{event.summary}</p></div>)}</div>}
  </aside>;
}
