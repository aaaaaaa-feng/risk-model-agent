...
export function App() {
  ...
  const loadSettings=useCallback(async()=>{try{const value=await api.get<any>("/providers/settings");setSettings({...value.settings, profiles:value.profiles||value.settings?.profiles||[], active_profile_id:value.active_profile_id||value.settings?.active_profile_id});}catch(error){notify(msg(error),true);}},[notify]);
