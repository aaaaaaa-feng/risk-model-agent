...
export function ReportView({ project, run }: Props) {
  ...
  useEffect(() => {
    ...
    setModelId("");
    reportsApi
      .models(project.id, controller.signal)
      .then((value) => {
        if (!active) return;
    ...
    if (run?.status === "succeeded")
      reportsApi
        .report(run.id, controller.signal)
        .then((value) => {
  ...
  const score = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uploaded = await reportsApi.uploadScoreInput(project.id, form);
      const result = await reportsApi.createScoreJob(modelId, uploaded.asset.id);
