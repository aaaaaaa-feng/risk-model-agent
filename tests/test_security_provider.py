...
@pytest.mark.parametrize(
    "value", ["api_key=secret-value", "Bearer abcdefghijklmnop", "sk-abcdefghijklmnop"]
)
...
def test_dlp_blocks_raw_rows_and_pii_values():
    ...
    with pytest.raises(ValueError, match="POSSIBLE_PII_VALUE_FORBIDDEN"):
        validate_safe_evidence({"summary": "请联系 13812345678"})


def test_small_cells_are_suppressed_at_30():
    rows = suppress_small_groups(
        [{"group": "a", "count": 29, "rate": 0.9}, {"group": "b", "count": 30, "rate": 0.2}]
    )
    assert rows[0] == {"count": 29, "suppressed": True}
    assert rows[1]["rate"] == 0.2
    outbound = sanitize_safe_evidence({"groups": [{"segment": "A", "count": 29, "bad_rate": 0.9}]})
    assert outbound["groups"] == [{"count": 29, "suppressed": True}]
...
def test_aes_gcm_bytes_and_streaming_recovery_key(tmp_path: Path):
    ...
    manifest, recovery = encrypt_file_payload(
        source, tmp_path / "encrypted.bin", "correct horse battery"
    )
    restored = decrypt_file_payload(
        tmp_path / "encrypted.bin", tmp_path / "restored.bin", manifest, recovery
    )
    assert restored.read_bytes() == source.read_bytes()
    assert manifest["cipher"] == "AES-256-GCM"


def test_archive_password_may_start_with_recovery_prefix(tmp_path: Path):
    source = tmp_path / "source.bin"
    source.write_bytes(b"local archive")
    password = "RMA-this-is-an-ordinary-password"
    manifest, _ = encrypt_file_payload(source, tmp_path / "encrypted.bin", password)
    restored = decrypt_file_payload(
        tmp_path / "encrypted.bin", tmp_path / "restored.bin", manifest, password
    )
    assert restored.read_bytes() == source.read_bytes()
...
def test_provider_presets_and_connectivity_without_raw_data(app_paths):
    ...
    gateway = ProviderGateway(
        settings, api_key="test-key-not-real", client_factory=FakeClient, paths=app_paths
    )
...
def test_gateway_suppresses_small_cells_and_fails_closed_on_value_pii(app_paths):
    ...
    result = gateway.complete("Return JSON.", {"groups": [{"count": 12, "bad_rate": 0.9}]})
    assert result.ok is True
    outbound = json.loads(FakeClient.requests[-1]["json"]["messages"][-1]["content"])
    assert outbound["groups"] == [{"count": 12, "suppressed": True}]
    blocked = gateway.complete("Return JSON.", {"summary": "13812345678"})
...
def test_openai_and_anthropic_payload_contracts(app_paths):
    openai = ProviderGateway(
        Settings(provider="openai", model="gpt-5"), api_key="x", paths=app_paths
    )
